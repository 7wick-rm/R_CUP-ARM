...
void MTCTaskNode::execute(const std::shared_ptr<GoalHandleArmCommand> goal_handle)
{
  ...
  auto block_names_it = BLOCK_MODEL_NAMES.find(goal->station);
  if (block_names_it != BLOCK_MODEL_NAMES.end() &&
      goal->block_slot >= 0 &&
      static_cast<size_t>(goal->block_slot) < block_names_it->second.size())
  {
    const std::string& gz_block_name = block_names_it->second[goal->block_slot];
    tf2::Transform block_pose;
    if (lookupLiveStationPose(gz_block_name, block_pose))
    {
      geometry_msgs::msg::Pose p;
      ...
      block.pick_x = p.position.x;
      block.pick_y = p.position.y;
      block.pick_z = p.position.z;
      block.pick_frame = "base_footprint";

      RCLCPP_INFO(LOGGER,
                  "Overrode pick pose for '%s' using live block TF '%s_live': "
                  "(%.3f, %.3f, %.3f)",
                  block.id.c_str(), gz_block_name.c_str(),
                  block.pick_x, block.pick_y, block.pick_z);
    }
    else
    {
      RCLCPP_WARN(LOGGER,
                  "Known block model '%s' for station '%s' slot %d but no live "
                  "TF for it -- using the executor-supplied pick pose instead.",
                  gz_block_name.c_str(), goal->station.c_str(), goal->block_slot);
    }
  }

  else if (goal->block_slot >= 0)
  {
