...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("piper_mtc_tutorial");
...
public:
  ...
  // Blocks until the warehouse arm publishes /piper/start_assembly -- lets the
  // whole existing setupPlanningScene()/runAssembly() sequence run completely
  // unchanged, just gated on when it begins instead of firing at process start.
  void waitForStartSignal();

  // Publishes /piper/assembly_done once runAssembly() has finished, so the
  // warehouse arm knows it's safe to come pick up the finished object.
...
private:
  ...
  std::future<void> start_future_{ start_promise_.get_future() };
  ...
  : node_{ std::make_shared<rclcpp::Node>("mtc_node", options) }
{
  ...
  start_sub_ = node_->create_subscription<std_msgs::msg::Empty>(
      "/piper/start_assembly", 10,
      [this](const std_msgs::msg::Empty::SharedPtr) {
        if (!start_received_.exchange(true))
        {
          RCLCPP_INFO(node_->get_logger(),
                      "Received /piper/start_assembly -- beginning assembly.");
          start_promise_.set_value();
        }
      });

  done_pub_ = node_->create_publisher<std_msgs::msg::Empty>("/piper/assembly_done", 10);
}

void MTCTaskNode::waitForStartSignal()
{
  RCLCPP_INFO(node_->get_logger(), "Waiting for /piper/start_assembly before beginning...");
  start_future_.wait();
}

void MTCTaskNode::signalAssemblyDone()
{
  RCLCPP_INFO(node_->get_logger(), "Assembly complete -- publishing /piper/assembly_done.");
...
int main(int argc, char** argv)
{
  ...
  mtc_task_node->waitForStartSignal();

  mtc_task_node->setupPlanningScene();
  mtc_task_node->runAssembly();
  mtc_task_node->signalAssemblyDone();
