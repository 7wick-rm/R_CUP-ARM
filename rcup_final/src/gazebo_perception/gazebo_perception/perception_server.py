...
class PerceptionServer(Node):
    def __init__(self):
        ...
        self.average_pose_service_topic = (
            self.get_parameter("average_pose_service_topic").get_parameter_value().string_value
        )
    ...
    async def handle_collection_request(self, request, response):
        ...
        self.sub = self.create_subscription(
            PerceptionMsg,
            "/perception/brick_data",
            self.pose_expanded_callback,
            10,
            callback_group=self.cb_group,
        )
    ...
    def pose_expanded_callback(self, msg):
        if len(msg.block_poses.poses) > 0:
            ...
            if (
                len(self.collected_frames) >= self.target_samples
                and not self.collect_future.done()
            ):
    ...
    def process_frames_and_average(self, collected_frames):
        ...
        for block_id, pose_list in id_dict.items():
            ...
            ) = (avg_quat[0], avg_quat[1], avg_quat[2], avg_quat[3])
